...
if ($reviewer_id != '') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (!empty($reviewerData['profile_image'])) {
            $reviewerData['profile_image'] = 'https://cmsa.digital/assets/profiles/profile_pics/' . $reviewerData['profile_image'] . '.jpg';
        }
        // Format the CV file URL if it exists
        if (!empty($reviewerData['rev_cv'])) {
            $reviewerData['rev_cv'] = 'https://cmsa.digital/assets/profiles/reviewer_cv/' . $reviewerData['rev_cv'] . '.pdf';
        }
