 ...
 * 
 * @param array $reviewer Reviewer details
 * @param string $newPassword The new password
 * @param string $confId Conference ID
 * @return bool Success or failure
 */
function sendPasswordResetEmail($reviewer, $newPassword, $confId) {
    try {
...
$mail = getConfiguredMailer(true);
        ...
        // Add recipient
        $mail->addAddress($reviewer['user_email'], $reviewer['user_name']);
        
        // Set email format to HTML
        $mail->isHTML(true);
        $mail->Subject = $confId . ' - Reviewer Password Reset';
        
        // Email body with professional template
        $mail->Body = getEmailTemplate($reviewer, $newPassword, $confId);
        
        // Plain text alternative
        $mail->AltBody = $confId . " - CMSA\n\n" .
            "You have requested a password reset. You can now login to the reviewer's page using the following credentials:\n\n" .
 ...
 */
function getEmailTemplate($reviewer, $newPassword, $confId) {
    return '
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <div class="alert">
                    <p>Your reviewer account password has been reset by an administrator.</p>
                </div>
                
                <p>Hello ' . $reviewer['user_name'] . ',</p>
                
                <p>You can now login to the reviewer\'s page using the following credentials:</p>
                
                <div class="credentials">
                    <p><span class="label">Email:</span> <span class="value">' . $reviewer['user_email'] . '</span></p>
...
// Get the reviewer ID and conference ID from the request
$reviewer_id = $_GET['reviewer_id'] ?? '';
$conf_id = $_GET['conf_id'] ?? '';

if (empty($reviewer_id) || empty($conf_id)) {
    echo json_encode(['status' => 'error', 'message' => 'Reviewer ID and Conference ID are required']);
    exit;
}
...
// Get reviewer details for the email
...
$emailSent = sendPasswordResetEmail($reviewer, $new_password, $conf_id);

if ($emailSent) {
    echo json_encode([
        'status' => 'success', 
        'message' => 'Password reset successfully. An email has been sent to the reviewer.'
    ]);
} else {
    echo json_encode([
        'status' => 'error',
        'message' => 'Password reset but failed to send email. Please inform the reviewer manually.'
    ]);
}
